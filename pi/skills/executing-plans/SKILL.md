...
**You MUST NOT call plan-mode tools during this skill.** This skill operates in normal mode, executing a plan that already exists on disk. Plan mode is unnecessary and dangerous here — it restricts Write/Edit tools needed for implementation.
...
**Note:** Tell your human partner that Superpowers works much better with access to subagents. The quality of its work will be significantly higher if run on a platform with subagent support (such as Pi). If subagents are available, use subagent-driven-development instead of this skill.

## The Process

### Step 0: Load Persisted Tasks

1. Call `TaskList` or the available Pi task-list tool to check for existing native tasks
...
4. Call `TaskList` or the available Pi task-list tool and verify blockedBy relationships show correctly (e.g., "blocked by #1, #2")
